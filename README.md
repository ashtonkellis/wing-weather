...
glanceable view and tells you whether conditions are inside your rideable
window.
...
- ✅ **Rideability check** — a min threshold per metric (tide, wind, and
  temperature are all min-only), shown at a glance. Thresholds live in
  `js/config.js`; there is no in-app editor
...
and auto-deployed on every push. Rideable ranges are set in `js/config.js`.
