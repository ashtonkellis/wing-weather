...
window.WW_CONFIG = {
  ...
  fallbackEndHour: 20,
  // Rideable ranges. Fixed here — there is no in-app editor, so this file is
  // the only place they change. A `null` max means "no upper limit".
  defaults: {
    tide: { min: 5.0, max: null, unit: "ft", label: "Tide height", icon: "🌊" },
    wind: { min: 7, max: null, unit: "kn", label: "Wind speed", icon: "💨" },
    temp: { min: 60, max: null, unit: "°F", label: "Temperature", icon: "🌡️" },
  },
};

// The rideable ranges every consumer runs on, as plain {min, max} per metric.
WW_CONFIG.thresholds = function () {
  const out = {};
  for (const m of Object.keys(WW_CONFIG.defaults)) {
    out[m] = { min: WW_CONFIG.defaults[m].min, max: WW_CONFIG.defaults[m].max };
  }
  return out;
};
